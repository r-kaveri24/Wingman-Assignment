...
const Table = () => {
  ...
  const getButtonState = (isDisabled) => {
    return {
      cursor: isDisabled ? "not-allowed" : "pointer",
      borderColor: isDisabled ? "#bbb" : "#115E56",
      backgroundColor: isDisabled ? "#f0f0f0" : "#fff",
      color: isDisabled ? "#bbb" : "#115E56",
    };
  };

  const prevButtonState = getButtonState(currentPage === 1);
  const nextButtonState = getButtonState(currentPage === totalPages);
    ...
    <div className="p-5 w-[100%] mb-3">
          ...
          <tbody className="overflow-hidden">
            {currentItems.map((item) => (
              <tr key={item.id}>
                ...
                <td className="p-[10px] text-base font-normal text-[#212636] ">
                  {item.date}{" "}
                  <p className="text-xs font-normal text-[#212636] ">
                    10:24 AM
                  </p>
                </td>
                <td className="p-[10px] text-base font-normal text-[#212636]">
                ...
                <td className="p-[10px] text-base font-bold text-[#212636]">
                  {item.commission}
                </td>
                <td className="p-[10px] text-base font-normal text-[#212636] flex justify-end">
                  <Link
                    href="/chat"
                    className="p-[10px] text-xs font-normal text-[#8A94A6] flex gap-3"
      ...
      >
        <button
          ...
          style={{
            padding: "10px 15px",
            border: `1px solid ${prevButtonState.borderColor}`,
            backgroundColor: prevButtonState.backgroundColor,
            color: prevButtonState.color,
            borderRadius: "8px",
            cursor: prevButtonState.cursor,
          }}
        >
          Previous
        </button>
        ...
        <button
          ...
          style={{
            padding: "10px 15px",
            border: `1px solid ${nextButtonState.borderColor}`,
            backgroundColor: nextButtonState.backgroundColor,
